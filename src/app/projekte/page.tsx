...
const STATUS_COLORS = {
  ...
  abgeschlossen: 'bg-[#2a2a2a] text-[#888]',
  pausiert:      'bg-[#d4e840]/15 text-[#d4e840]',
}
      ...
      <div className="max-w-6xl mx-auto px-6 py-6">
        {loading ? (
          ...
          <div className="border border-dashed border-[#2a2a2a] rounded-2xl p-16 text-center">
            <p className="text-[#444] mb-3">Noch keine Projekte</p>
              ...
              <Link key={p.id} href={`/projekte/${p.id}`}
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-sm text-[#555] mt-0.5 truncate">{p.kunde_name}</p>
                  </div>
                ...
                {p.beschreibung && (
                  <p className="text-xs text-[#555] mb-4 line-clamp-2">{p.beschreibung}</p>
                )}
                <div className="flex items-center justify-between text-xs text-[#555] pt-3 border-t border-[#2a2a2a]">
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-6 z-50" onClick={e => e.target === e.currentTarget && setNeuDialog(false)}>
          <div className="bg-[#181818] border border-[#2a2a2a] rounded-2xl p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs text-[#666] mb-1.5 block">Beschreibung</label>
                <textarea value={form.beschreibung} onChange={e => setForm({ ...form, beschreibung: e.target.value })}
                  ...
                  className="w-full bg-[#111] border border-[#2a2a2a] rounded-xl px-4 py-3 text-sm text-[#f0ede8] placeholder-[#444] focus:outline-none focus:border-[#d4e840] resize-none transition-colors"/>
              </div>
            </div>
            <div className="flex gap-3 mt-5">
              <button type="button" onClick={() => setNeuDialog(false)}
                className="flex-1 py-3 rounded-xl border border-[#2a2a2a] text-sm text-[#888] hover:text-[#f0ede8] hover:border-[#444] transition-all">
                Abbrechen
              </button>
